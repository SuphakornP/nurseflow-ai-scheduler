...
export async function suggestNormalizations(periodId: string, items: AmbiguousValue[]) {
  ...
  const response = await client.responses.parse({
    ...
    instructions:
      "Normalize only the provided nurse scheduling tokens. Never infer employee identity or personal information. Vacation and Education are locked. O1-O4 are OFF priorities. O/D and O/N are flexible allowed sets. Mark every non-standard token for human review even when the interpretation is likely. Preserve each input id exactly.",
